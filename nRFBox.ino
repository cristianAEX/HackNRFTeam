...
// ─────────────────────────────────────────────
//  Menú — 14 ítems (12 originales + 2 Sub-GHz)
// ─────────────────────────────────────────────
const int NUM_ITEMS       = 14;
const int MAX_ITEM_LENGTH = 20;

const unsigned char* bitmap_icons[NUM_ITEMS] = {
  bitmap_icon_scanner,    bitmap_icon_analyzer,  bitmap_icon_jammer,
  bitmap_icon_kill,       bitmap_icon_ble_jammer,bitmap_icon_spoofer,
  bitmap_icon_apple,      bitmap_icon_ble,       bitmap_icon_wifi,
  bitmap_icon_wifi_jammer,
  // Íconos Sub-GHz (nuevos)
  bitmap_icon_subghz_rx,  bitmap_icon_subghz_tx,
  bitmap_icon_about,      bitmap_icon_setting
};

char menu_items[NUM_ITEMS][MAX_ITEM_LENGTH] = {
  "Scanner",    "Analyzer",    "WLAN Jammer", "Proto Kill",
  "BLE Jammer", "BLE Spoofer", "Sour Apple",  "BLE Scan",
  "WiFi Scan",  "Deauther",
  // Sub-GHz CC1101
  "SubGHz Cap", "SubGHz Replay",
  "About",      "Setting"
};

void (*menu_functions[NUM_ITEMS])() = {
  Scanner::scannerSetup,       Analyzer::analyzerSetup,
  Jammer::jammerSetup,         ProtoKill::blackoutSetup,
  BleJammer::blejammerSetup,   Spoofer::spooferSetup,
  SourApple::sourappleSetup,   BleScan::blescanSetup,
  WifiScan::wifiscanSetup,     Deauther::deautherSetup,
  // Sub-GHz CC1101
  SubGhzCapture::subghzCaptureSetup,
  SubGhzReplay::subghzReplaySetup,
  utils,                       Setting::settingSetup
};

void (*menu_loop_functions[NUM_ITEMS])() = {
  Scanner::scannerLoop,        Analyzer::analyzerLoop,
  Jammer::jammerLoop,          ProtoKill::blackoutLoop,
  BleJammer::blejammerLoop,    Spoofer::spooferLoop,
  SourApple::sourappleLoop,    BleScan::blescanLoop,
  WifiScan::wifiscanLoop,      Deauther::deautherLoop,
  // Sub-GHz CC1101
  SubGhzCapture::subghzCaptureLoop,
  SubGhzReplay::subghzReplayLoop,
...
void drawMenu() {
  ...
  // Barra superior con nombre del equipo y versión
  ...
  // Mostrar "Team NRF24" en la barra superior
  ...
  if (start_row > 0)              u8g2.drawStr(124, 15, ".");
  if (start_row < total_rows - 1) u8g2.drawStr(124, 64, ".");
...
bool readButton(int pin) {
  if (digitalRead(pin) == LOW &&
      (millis() - last_button_time > DEBOUNCE_DELAY)) {
...
void setup() {
  ...
  conf();   // splash screen → mostrará "Team NRF24"
